...
# Remove the second row of column names
second_row_index = df.columns.duplicated().argmax()
df = df.drop(second_row_index)
...
matrix_names = [name.replace('test_subjects/mm/', '') for name in matrix_names]
...
mem_bar = ax.bar(matrix_names, df_combined_sorted['Mem Wall Time'], alpha=0.5, log=True, width=bar_width, label='Mem Wall Time', color='red')

# Plot the fault time bars
fault_bar = ax.bar(matrix_names, df_combined_sorted['Fault Time'], width=bar_width,
                   bottom=df_combined_sorted['Mem Wall Time'], alpha=0.5, color='orange',label='Fault Time', log=True)
...
gpu_bar = ax.bar(matrix_names, df_combined_sorted['GPU Wall Time'], width=bar_width,
                 ...
                 label='GPU Wall Time', alpha=0.5, color='blue', log=True)
...
plt.tight_layout()
plt.savefig('Data/gpu_time_sections.svg', format='svg')
plt.show()
plt.close()
...
iteration_bar = ax.bar(matrix_names, df_combined_sorted['CPU Iteration Count'], width=bar_width, label='Iteration Count', alpha=0.5,color='blue')
iteration_bar = ax.bar(index+bar_width*2, df_combined_sorted['GPU Iteration Count'], width=bar_width, label='Iteration Count', alpha=0.5,color ='green')
